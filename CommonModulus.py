"""
@Author Reyce Salisbury

Tool for breaking RSA when you have two messages with a common modulus
(The two ciphertexts are different in each message, however the plaintext
message for the two encrypted messages should be the same)

INPUT VALUES MUST BE IN HEX!
"""
import argparse

#Example values (Values from Hack The Box's Cyber Santa is Coming to Town Challenge: Common Mistake)
#ct1 = ''
#e1 = '0x10001'
#ct2 = ''
#e2 = '0x23'
#n = ''

parser = argparse.ArgumentParser(description="Common Mod Attack")
needed_vals = parser.add_argument_group("needed values for attack")
needed_vals.add_argument('-ct1', '--ciphertext1', help="Ciphertext 1", type=str, required=True)
needed_vals.add_argument('-e1', '--exponent1', help="Exponent for Ciphertext 1", type=str, required=True)
needed_vals.add_argument('-ct2', '--ciphertext2', help="Ciphertext 2", type=str, required=True)
needed_vals.add_argument('-e2', '--exponent2', help="Exponent for Ciphertext 2", type=str, required=True)
needed_vals.add_argument('-n', '--modulus', help="N (Modulus) value", type=str, required=True)


#euclidean extended gcd for e values
def egcd(a,b):
    if a == 0:
        return b, 0, 1
    g, x1, y1 = egcd(b%a, a)

    x = y1 - (b//a) * x1
    y = x1

    return g, x, y


def main():
    args = parser.parse_args()

    ct1bt = int(args.ciphertext1, 16)
    e1bt = int(args.exponent1, 16)
    ct2bt = int(args.ciphertext2, 16)
    e2bt = int(args.exponent2, 16)
    nbt = int(args.modulus, 16)

    g, a1, a2 = egcd(e1bt, e2bt)
    if g != 1:
        raise ValueError("The exponents of each ciphertext have to be coprime! (gcd(e1, e2) must equal 1)")

    #main attack:
    message = ((pow(ct1bt, a1, nbt)) * (pow(ct2bt, a2, nbt))) % nbt
    print("\nPlaintext Message: \n" + hex(message))

main()